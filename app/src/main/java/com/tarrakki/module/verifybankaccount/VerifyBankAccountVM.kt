...
class VerifyBankAccountVM : FragmentViewModel() {
    ...
    fun updateBankDetails(userBankData: UserBanksResponse, bankId: String): MutableLiveData<UserBanksResponse> {
        ...
        subscribeToSingle(
                observable = ApiClient.getHeaderClient().create(WebserviceBuilder::class.java).updateUserBank(
                        userBankData.data.bankDetail.accountNumber,
                        userBankData.data.bankDetail.ifsc_code,
                        userBankData.data.bankDetail.accountTypeBse,
                        App.INSTANCE.getUserId(),
                        bankId,
                        userBankData.data.bankDetail.id.toString()),
